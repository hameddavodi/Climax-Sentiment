## Intro
The movie "Climax 2018" directed by Gasper Noe has been the subject of much critical acclaim and debate. To delve deeper into the movie's impact and meaning, I undertook a project to analyze it from three perspectives: Scripts, Colors, and Sound analysis, using Python. In the text analysis, I examined the screenplay and dialogue for patterns and themes, while the Color analysis focused on decomoposing the colors of frames and making a pallets of dominant colors. Lastly, I explored the music in the film, analyzing its emotional impact and how it enhances the viewing experience. Through this comprehensive analysis, I aimed to gain a deeper understanding of the complex themes and emotions presented in the film.

<em>Note that the texts provided here are generated by chatgpt under my confirmation of details and context and analysis. So, basically, it helped me to speed up the process of writing</em> .


### 1- Collecting Data

The code is intended to extract the text from the SRT file, perform sentiment analysis on the text using TextBlob, and store the sentiment scores for each sentence in a list.
...
Lets check the data with histograms of polarity and subjectivity.
...
The code is analyzing two sentiment features: polarity and subjectivity.
...
As indicated above, we can see the symmetric distribution of polarity with a high frequency of neutral sentiments. In addition, the symmetry also indicates of creating tension, and suspense, or conveys a deeper message
...
We may see that the sentiment of the sentences is equally distributed. However, let's plot the regression line!!
...
The linear regression red line indicates that during the movie, the average sentiments of the sentences become more and more negative. 
...
- Plot the actual 'compound' values with color-coded cluster labels and finally
...
This means that averaged predicted compound sentiments of sentences in the movie fluctuated and touched the extreme border points of the neutral sentiment section.
...
I didn't sort the word data, so their order inherently represents the timeline. Therefore, based on the figure above, we can see that although the trend of negative words fluctuated severely alongside positive words, it still increased over time.
...
I used cv2 lib to calculate the dominant colors for each frame of the movie.
```python
import cv2
import numpy as np
from sklearn.cluster import KMeans
...
I wanted to use a wide range of research about classifying the emotions of the music by decomposing different " scales " in music but as I went through I
 found that it is almost a kind of college major to do so. Eventually, I summarized some feature extractions used in the 'librosa' library to simplify the analysis. 
...
Furthermore, onset strength is a concept used in the field of music psychology to describe the perceived intensity or strength of the beginning of a sound or musical note. It refers to the rate of change in sound pressure level at the onset of a sound.
...
The high frequency content of music can sometimes extend beyond 4 kHz and even reach up to 8 kHz or higher as I can see in the middle of movie the freqs went beyond 8k Hz. The specific range of high frequencies that contribute to the emotional expression of music can vary depending on the musical genre, cultural context, and individual preferences.
